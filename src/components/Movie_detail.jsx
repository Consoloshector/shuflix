...
export default function MovieDetail() {
    ...
    if (!clicked_film) {
        return (
            <div className="h-[90vh] flex items-center justify-center bg-black">
                <div><h1 className="text-white">Yüklənir....</h1></div>
            </div>
        );
    }
    ...
    return (
        <div className="relative flex flex-col w-full min-h-[90vh] bg-black">
            ...
            <div className="relative z-10 max-w-5xl flex flex-col pt-5 text-zinc-300 text-sm items-center gap-4 sm:flex-row sm:mx-auto">
                <img className="max-w-[200px] max-h-[300px] mt-10 object-cover rounded-2xl md:max-w-[250px] md:max-h-[350px] sm:mt-0 lg:max-w-[350px] lg:max-h-[450px]" src={`https://image.tmdb.org/t/p/w500${clicked_film.poster_path}`} alt="" />
                <div className="flex flex-col items-center text-center gap-3 px-5 sm:text-left sm:items-start">
                    <h1 className="font-bold text-2xl text-white lg:text-5xl">{clicked_film.title}</h1>
                    <div className="flex gap-1 sm:text-lg items-center">
                        <p className="bg-red-500/55 border-2 border-red-500/30 p-1 rounded font-bold text-xs">{clicked_film.release_date?.split("-")[0]}</p>●
                        <p className="bg-green-500/55 border-2 border-green-500/30 p-1 rounded font-bold text-xs"> {Math.floor(clicked_film.runtime / 60)}s {clicked_film.runtime % 60}dq</p>
                    </div>
